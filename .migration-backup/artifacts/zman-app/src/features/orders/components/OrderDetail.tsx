...
import { useDeleteOrder, useOrder, usePatchOrderStatus } from "../hooks";
import { ORDER_STATUSES, orderStatusLabels } from "../schema";
...
export function OrderDetail({ orderId, onEdit, onBack }: OrderDetailProps) {
  ...
  const patchStatusMutation = usePatchOrderStatus();
  const [isConverting, setIsConverting] = useState(false);

  const handleStatusChange = async (newStatus: OrderStatus) => {
    const response = await patchStatusMutation.mutateAsync({
      id: orderData?.id ?? "",
      updatedAt: orderData?.updatedAt ?? "",
      status: newStatus,
    });
    if (response.status === "ok") {
      toast.success(`تم تغيير الحالة إلى: ${orderStatusLabels[newStatus]}`);
    } else {
      toast.error(response.message || "فشل تغيير الحالة");
    }
  };
  ...
  const handleCopyOrderSummary = async () => {
    const lines = [
      ...
      orderData.customerPhone ? `الهاتف: ${orderData.customerPhone}` : null,
      `المنتج: ${orderData.productName}`,
      `الكمية: ${orderData.quantity} قطعة`,
      `السعر: ${(orderData.totalPriceCents / 1000).toFixed(3)} د.أ`,
      orderData.notes ? `ملاحظات: ${orderData.notes}` : null,
    ].filter(Boolean).join("\n");
    ...
    <div className="space-y-6 max-w-xl mx-auto pb-24 lg:pb-0">
          ...
          <div>
            ...
            {orderData.customerPhone && (
              <a
                ...
                className="text-sm text-info block mt-1 hover:underline"
                dir="ltr"
              >
                {orderData.customerPhone}
              </a>
      ...
      {orderData.status !== "delivered" && orderData.status !== "cancelled" && (
        <div className="bg-paper p-5 rounded-lg border border-hairline shadow-sm space-y-3">
          <h4 className="text-sm font-bold text-ink-2">تحديث حالة الطلب</h4>
          <div className="flex flex-wrap gap-2">
            {ORDER_STATUSES.filter((s) => s !== orderData.status && s !== "draft").map((s) => {
              const isDestructive = s === "cancelled";
              const isCurrent = s === orderData.status;
              return (
                <button
                  key={s}
                  type="button"
                  disabled={isCurrent || patchStatusMutation.isPending}
                  onClick={() => handleStatusChange(s)}
                  className={cn(
                    "min-h-[36px] px-3 py-1.5 rounded-full text-xs font-semibold border transition-colors disabled:opacity-40",
                    isCurrent
                      ? "bg-info-soft text-info border-info/20 cursor-default"
                      : isDestructive
                        ? "border-alert/30 text-alert hover:bg-alert-soft"
                        : "border-hairline-2 text-ink-2 hover:bg-canvas hover:border-ink/20",
                  )}
                >
                  {patchStatusMutation.isPending ? "..." : orderStatusLabels[s]}
                </button>
              );
            })}
          </div>
              ...
              >
                <div>
                  ...
                  <span className="text-xs text-ink-3 block">
                    {c.quantity} × <AmountText amount={c.unitCostCents} />
                  </span>
                </div>
                <span className="font-bold text-ink-2">
                  <AmountText amount={c.unitCostCents * c.quantity} />
                </span>
              </div>
            ))}
      ...
      <div className="fixed bottom-0 inset-x-0 p-4 bg-paper/90 backdrop-blur-md border-t border-hairline flex flex-col gap-3 lg:static lg:p-0 lg:bg-transparent lg:border-none z-sticky lg:z-auto">
        <div className="flex gap-2">
          <button
            type="button"
            onClick={handleWhatsApp}
            className="flex-1 min-h-[44px] py-3 px-4 rounded-md bg-info text-paper font-bold hover:bg-info/90 transition-colors flex items-center justify-center gap-2"
          >
          ...
          <button
            ...
            className="min-h-[44px] min-w-[44px] px-4 rounded-md border border-hairline-2 text-ink-2 hover:bg-canvas transition-colors flex items-center justify-center gap-2"
            title="نسخ ملخص الطلب"
          >
            <Clipboard className="w-5 h-5" />
            <span className="hidden sm:inline text-sm font-semibold">نسخ الملخص</span>
          </button>
        </div>
