    ...
    <AppShell
      ...
      action={
        <div className="flex gap-2">
          {/* منتقي الفترة للأجهزة المكتبية (Inline) */}
          <div className="hidden lg:flex gap-1.5 p-1 bg-canvas rounded-md border border-hairline">
            {presets.map((preset, i) => (
              <button
                key={preset.label}
                type="button"
                onClick={() => handlePresetSelect(i)}
                className={`px-3 min-h-[44px] rounded text-xs font-bold transition-all ${
          ...
          {/* زر منتقي الفترة للموبايل */}
          <button
            type="button"
            onClick={() => setIsSelectorOpen(true)}
            className="lg:hidden h-11 min-h-[44px] px-3 bg-paper border border-hairline text-ink rounded-md flex items-center gap-1.5 text-xs font-bold"
          >
            <Calendar className="h-4 w-4 text-info" />
            <span>
              {customRange
                ? "فترة مخصصة"
                : (presets[selectedPresetIdx]?.label ?? "")}
            </span>
          </button>
        </div>
      }
    >
      <div className="space-y-6">
        ...
        {isLoadingSummary ? (
          ...
          <div className="space-y-4">
            ...
            <div className="p-6 bg-paper rounded-lg border border-hairline shadow-sm flex flex-col justify-between w-full">
              ...
              <div className="mt-4">
                <span
                  className={`text-2xl lg:text-4xl font-bold flex items-center gap-1.5 ${netColorClass}`}
                >
                  <span className="font-mono">{netSign}</span>
                  <AmountText amount={Math.abs(net)} />
                </span>
                <p className="text-xs text-ink/50 mt-2">
                  الإيرادات − التكاليف والمصاريف التشغيلية
                </p>
              </div>
            </div>
        ...
        {!isLoadingTrend && trendData && (
          <Suspense fallback={<div className="h-64 w-full bg-paper rounded-lg border border-hairline animate-pulse" />}>
            <FinancialChart salesData={trendData.salesTrend} expensesData={trendData.expensesTrend} purchasesData={trendData.purchasesTrend} startDate={startDateStr} endDate={endDateStr} />
            </Suspense>
        )}
      ...
      <button
        ...
        className="fixed bottom-24 inset-s-6 lg:hidden z-sticky h-14 w-14 rounded-full bg-ink text-paper shadow-lg flex items-center justify-center hover:bg-ink/90 active:scale-95 transition-all"
